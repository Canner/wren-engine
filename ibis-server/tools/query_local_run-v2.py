...
from app.model.data_source import BigQueryConnectionInfo, DataSource
...
async def main():
    ...
    print("#")
    rewriter = Rewriter(encoded_str,
                         data_source=DataSource[data_source],
                           java_engine_connector=JavaEngineConnector(os.getenv("WREN_ENGINE_ENDPOINT")))
    # session_context = SessionContext(encoded_str, function_list_path)
    # planned_sql = session_context.transform_sql(sql)
    planned_sql = await rewriter.rewrite(sql)
    print("# Planned SQL:\n", planned_sql)
    ...
    if data_source == "bigquery":
        ...
        df = connection.sql(planned_sql).limit(10).to_pandas()
        print("### Result ###")
        print("")
        print(df)
    else:
