...
def minio(request) -> MinioContainer:
    ...
    client.fput_object(
        bucket, "tpch/data/orders.parquet", "tests/resource/tpch/data/orders.parquet"
    )
    client.fput_object(
        bucket,
        "tpch/data/customer.parquet",
        "tests/resource/tpch/data/customer.parquet",
    )

    client.fput_object(
        bucket,
        "test_file_source/type-test-parquet/t1.parquet",
        "tests/resource/test_file_source/type-test-parquet/t1.parquet",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test-parquet/t2.parquet",
        "tests/resource/test_file_source/type-test-parquet/t2.parquet",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test-csv/t1.csv",
        "tests/resource/test_file_source/type-test-csv/t1.csv",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test-csv/t2.csv",
        "tests/resource/test_file_source/type-test-csv/t2.csv",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test-json/t1.json",
        "tests/resource/test_file_source/type-test-json/t1.json",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test-json/t2.json",
        "tests/resource/test_file_source/type-test-json/t2.json",
    )
    client.fput_object(
        bucket, "test_file_source/invalid", "tests/resource/test_file_source/invalid"
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test.csv",
        "tests/resource/test_file_source/type-test.csv",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test.json",
        "tests/resource/test_file_source/type-test.json",
    )
    client.fput_object(
        bucket,
        "test_file_source/type-test.parquet",
        "tests/resource/test_file_source/type-test.parquet",
    )

    request.addfinalizer(minio.stop)
    return minio


async def test_query(client, manifest_str, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
...
async def test_query_with_limit(client, manifest_str, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
        f"{base_url}/query",
        params={"limit": 1},
...
async def test_query_calculated_field(client, manifest_str, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
        f"{base_url}/query",
        json={
...
async def test_dry_run(client, manifest_str, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
        f"{base_url}/query",
        params={"dryRun": True},
...
async def test_query_with_invalid_connection_info(client, manifest_str, minio):
    response = await client.post(
        f"{base_url}/query",
        json={
            "manifestStr": manifest_str,
...
async def test_metadata_list_tables(client, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
        url=f"{base_url}/metadata/tables",
        json={
...
async def test_metadata_list_constraints(client, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
...
async def test_metadata_db_version(client, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "parquet")
    response = await client.post(
    ...
    assert response.status_code == 200
    assert "Minio" in response.text


async def test_unsupported_format(client, minio):
    connection_info = _to_connection_info(minio, "/tpch/data", "unsupported")
    response = await client.post(
        url=f"{base_url}/metadata/tables",
        json={
            "connectionInfo": connection_info,
...
async def test_list_parquet_files(client, minio):
    connection_info = _to_connection_info(minio, "/test_file_source", "parquet")
    response = await client.post(
        url=f"{base_url}/metadata/tables",
        json={
            "connectionInfo": connection_info,
...
async def test_list_csv_files(client, minio):
    connection_info = _to_connection_info(minio, "/test_file_source", "csv")
    response = await client.post(
        url=f"{base_url}/metadata/tables",
        json={
            "connectionInfo": connection_info,
...
async def test_list_json_files(client, minio):
    connection_info = _to_connection_info(minio, "/test_file_source", "json")
    response = await client.post(
        url=f"{base_url}/metadata/tables",
        json={
            "connectionInfo": connection_info,
...
def _to_connection_info(minio: MinioContainer, root, format):
