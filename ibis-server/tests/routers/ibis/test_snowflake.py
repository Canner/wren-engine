...
class TestSnowflake:
    ...
    def test_query(self):
        ...
        assert result['data'][0] == [1, 36901, 'O', 173665.47, 820540800000, '1_36901', 1704153599000, 1704153599000]
    ...
    def test_query_with_column_dtypes(self):
        ...
        assert len(result['columns']) == len(self.manifest['models'][0]['columns'])
        assert len(result['data']) == 1
        assert result['data'][0] == [1, 36901, 'O', 173665.47, '1996-01-02 00:00:00.000000', '1_36901', '2024-01-01 23:59:59.000000', '2024-01-01 23:59:59.000000 UTC']
        assert result['dtypes'] == {
            'orderkey': 'int64',
            'custkey': 'int64',
            'orderstatus': 'object',
