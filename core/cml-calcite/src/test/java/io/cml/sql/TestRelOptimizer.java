...
import io.cml.sql.calcite.CmlTable;
...
public class TestRelOptimizer
{
    ...
    public TestRelOptimizer()
    {
        ...
        for (TpchTable table : TpchTable.values()) {
            ...
            schema.add(table.name().toLowerCase(Locale.ROOT), new CmlTable(table.name().toLowerCase(), builder.build()));
            cmlTableBuilder.put(table.name().toLowerCase(), new CmlTable(table.name().toLowerCase(), builder.build()));
        }
