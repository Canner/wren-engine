...
import static io.cml.sql.analyzer.ExpressionTreeUtils.asQualifiedName;
import static io.cml.sql.analyzer.SemanticExceptions.ambiguousAttributeException;
import static io.cml.sql.analyzer.SemanticExceptions.missingAttributeException;
...
public class Scope
{
    ...
    public Optional<AsteriskedIdentifierChainBasis> resolveAsteriskedIdentifierChainBasis(QualifiedName identifierChain, AllColumns selectItem)
    {
        ...
        if (scopeForTableReference.isPresent()) {
            return Optional.of(new AsteriskedIdentifierChainBasis(BasisType.TABLE, scopeForTableReference, Optional.of(scopeForTableReference.get().getRelationType())));
        }
    ...
    private Optional<ResolvedField> resolveField(Expression node, QualifiedName name, boolean local)
    {
        ...
        if (matches.size() > 1) {
            List<Field> visibleMatches = matches.stream()
            ...
            if (visibleMatches.size() == 1) {
                int parentFieldCount = getLocalParent()
                        .map(Scope::getLocalScopeFieldCount)
                        .orElse(0);
                return Optional.of(asResolvedField(getOnlyElement(visibleMatches), parentFieldCount, local));
            }
