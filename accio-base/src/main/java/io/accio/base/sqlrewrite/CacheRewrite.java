...
import io.accio.base.sqlrewrite.analyzer.CacheAnalysis;
...
import static io.accio.base.sqlrewrite.Utils.analyzeFrom;
...
public class CacheRewrite
{
            ...
            AccioMDL accioMDL)
    {
        try {
            ...
            Statement rewritten = (Statement) new Rewriter(sessionContext, converter, accioMDL, aggregationAnalysis).process(statement, Optional.empty());
            ...
            extends BaseRewriter<Optional<Scope>>
    {
        ...
        protected Node visitQuerySpecification(QuerySpecification node, Optional<Scope> scope)
        {
            ...
            if (node.getFrom().isPresent()) {
                relationScope = Optional.of(analyzeFrom(accioMDL, sessionContext, node.getFrom().get(), scope));
            }
            else {
                relationScope = scope;
            }
        ...
        protected Node visitTable(Table node, Optional<Scope> scope)
        {
            ...
            if (accioMDL.getCacheInfo(catalogSchemaTableName).isPresent()) {
