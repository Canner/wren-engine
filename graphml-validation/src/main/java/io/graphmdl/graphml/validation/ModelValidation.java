...
import io.graphmdl.graphml.base.dto.EnumDefinition;
import io.graphmdl.graphml.base.dto.Model;
import io.graphmdl.graphml.connector.Client;
import io.graphmdl.graphml.connector.ColumnDescription;
...
import static io.graphmdl.graphml.validation.ValidationResult.error;
import static io.graphmdl.graphml.validation.ValidationResult.fail;
import static io.graphmdl.graphml.validation.ValidationResult.formatRuleWithIdentifier;
    ...
    private CompletableFuture<ValidationResult> validateModel(Client client, Model model, List<EnumDefinition> enumDefinitions)
    {
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (columnDescriptions.isEmpty()) {
                return error(formatRuleWithIdentifier(RULE_NAME, model.getName()),
            ...
            if (validationFaileds.isEmpty()) {
                return pass(formatRuleWithIdentifier(RULE_NAME, model.getName()), Duration.of(System.currentTimeMillis() - start, ChronoUnit.MILLIS));
            }

            return fail(formatRuleWithIdentifier(RULE_NAME, model.getName()),
