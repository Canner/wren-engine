...
import static io.graphmdl.sqlrewrite.GraphMDLFunctionSqlRewrite.GRAPHMDL_FUNCTION_SQL_REWRITE;
import static io.graphmdl.sqlrewrite.Utils.parseSql;
import static io.trino.sql.SqlFormatter.formatSql;
import static org.assertj.core.api.Assertions.assertThat;

public class TestGraphMDLFunctionSqlRewrite
{
    @DataProvider
    public Object[][] rewrite()
    {
        return new Object[][] {
                ...
                {"SELECT concat(any(filter(rs, rs -> rs.col1 = 'F')).col1, 'foo') AS a FROM model",
                        "SELECT concat(filter(rs, rs -> rs.col1 = 'F')[1].col1, 'foo') AS a FROM model"},
                {"SELECT first(filter(rs, rs -> rs.col1 = 'F'), col, ASC).col1 AS a FROM model",
                        "SELECT array_sort(filter(rs, rs -> rs.col1 = 'F'), col, ASC)[1].col1 AS a FROM model"},
        };
    }
    ...
    {
        assertThat(formatSql(GRAPHMDL_FUNCTION_SQL_REWRITE.apply(parseSql(original), null, null)))
