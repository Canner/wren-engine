        ...
        try {
            ...
            if (INVALID_QUERY.contains(e.getReason())) {
                if (e.getMessage().contains("ambiguous at")) {
                    throw new AccioException(AMBIGUOUS_NAME, "There are ambiguous column names", e);
                }
                throw new AccioException(GENERIC_USER_ERROR, format("Invalid statement: %s", query), e);
            }
